...
class AuthenticSourceLoginInput(BaseModel):
    user_id: str


def create_document(user_id: str) -> str:
    print(f"creating document for user_id {user_id}")

    pdf = """JVBERi0xLjMNCiXi48/TDQoNCjEgMCBvYmoNCjw8DQovVHlwZSAvQ2F0YWxvZw0KL091dGxpbmVz
...
async def authentic_source_login(
    redis_conn: Redis[str], authentic_source_login_input: AuthenticSourceLoginInput
) -> str:
    """Check credential"""

    if not authentic_source_login_input.user_id.isalnum():
